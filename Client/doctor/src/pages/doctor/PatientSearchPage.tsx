...
import { Search, User, IdCard, ChevronRight, X, Loader2 } from "lucide-react";
...
export function PatientSearchPage() {
  ...
  const [allPatients, setAllPatients] = useState<QueueEntry[]>([]);
  const [loading, setLoading] = useState(true);

  // Load today's queue as the searchable dataset
  useEffect(() => {
    async function load() {
      try {
        setLoading(true);
        const data = await fetchDoctorQueue();
        setAllPatients(data.queue);
      } catch {
        // Silently fail, search will show empty
      } finally {
        setLoading(false);
      }
    }
        ...
        {!loading && q.length < 2 && (
          <div className="flex flex-col items-center gap-3 py-16 text-center">
            ...
            <p className="max-w-xs text-base text-ink-muted">
              Type at least 2 characters to search by name, ABHA ID, or
        ...
        {!loading && q.length >= 2 && results.length === 0 && (
          <div className="flex flex-col items-center gap-3 py-16 text-center">
            ...
            <p className="text-base text-ink-muted">
              No match for "<strong>{query}</strong>" in your
              queue.
            </p>
          </div>
        )}
          ...
          <div className="flex flex-col gap-2">
            ...
            {results.map((patient) => (
              <button
                ...
                onClick={() =>
                  navigate(
                    `/doctor/patients/${patient.sessionId}/report`
                  )
                }
